<template>
	<section>
		<div v-title>添加商品</div>
		<div class="delete"><span class="detail">奖品信息</span><span @click="Delete" class="goDelete">删除</span></div>
		<div class="line"></div>
		<div class="rowAll">
			<!--  -->
		    <div class="row">
		    	...
		    	<div class="weui-cell__bd">
		    		<input class="input" v-model="name" autofocus type="text" placeholder="请输入商品名称"/>	    
		    	</div>
		    </div>
		    <div class="line"></div>		    
		    <div class="row">
		...
		<modals v-if="(btnShow||deleShow)&&mod" :title="btnShow?'确认修改?':'确认删除?'"></modals>
		<victory v-if="showSuc" victory="true" :behavior="btnShow?'修改成功':'删除成功'"></victory>
		<div class="bg" v-if="tipsNo">
			<div class="recharge_no">
				<p class="bg_tips">{{fail}}</p>
...
import Victory from '../../../components/victory/victory'
	export default{
		...
		methods:{
			query(){
				this.zhuan = true;
				this.$http.post(_config.server+'/shop/goods/detail',{id:this.$route.query._id}).then(res=>{
					// console.log(res)
					if(res.status==200&&res.data.status==10000){
						this.zhuan = false;
						this.name = res.data.data.name;
						this.price = res.data.data.priceNum/1200+'';
						this.num = res.data.data.num+'';
						this.summary = res.data.data.summary;
						this.cdn_url = res.data.data.thumb;
						this.shelves = res.data.data.shelves;
					}
				})
			},
			sub(){
			...
			upload(){
				...
				console.log(_config.server+'/shop/goods/update')
				console.log(params)
				this.zhuan = true;
				this.$http.post(_config.server+'/shop/goods/update',params).then(res=>{
					console.log(res)
					if(res.status==200&&res.data.status==10000){
						this.zhuan = false;
						this.showSuc = true;
			...
			goDelete(){
				...
				this.zhuan = true;
				this.$http.post(_config.server+'/shop/goods/delete',{id:this.$route.query._id}).then(res=>{
					// console.log(res)
					if(res.status==200&&res.data.status==10000){
						this.zhuan = false;
						this.showSuc = true;
